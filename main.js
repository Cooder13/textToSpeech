...
const textToSpeech=()=>{
    ...
    if(synth.speaking && speaking){
        textToSpeechBtnEl.innerHTML="Pause";
        synth.resume();
        speaking=false;
        speakerIconEl.innerHTML="&#128266";
    }
    else{
        textToSpeechBtnEl.innerHTML="Resume";
        synth.pause();
        speaking=true;
        speakerIconEl.innerHTML="&#128264"
    }

    setInterval(()=>{
        if(!synth.speaking && !speaking){
            speaking=true;
            textToSpeechBtnEl.innerHTML='Text to Speech'
        }
    })
};
